...
var git_path = path.join(process.cwd(), '.git');
if (fs.lstatSync(git_path).isFile()) {
    var git_file_contents = fs.readFileSync(git_path, {encoding: 'utf8'}).substring(8).trim();
    console.log(git_file_contents);
    git_path = path.join(process.cwd(), fs.readFileSync(git_path, {encoding: 'utf8'}).substring(8).trim());
}
