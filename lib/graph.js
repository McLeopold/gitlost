...
function graph(settings) {
    ...
    .then(function (branches) {
        ...
        var rev_list_cmds = settings.branches.map(function (branch) {
            return queue_cmd('git rev-list --max-parents=0 ' + branch)
            .then(function (first_rev) {
                var first_carot = first_rev == vars.base_commit
                    ? ''
                    : carot;
                return queue_cmd('git rev-list --parents --first-parent --pretty=oneline ' + branch + ' ' + first_carot + vars.base_commit + first_carot)
