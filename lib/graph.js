...
/*
 * Memoize results until the watch endpoint returns a change,
 * then stupidly invalidate everything per repo.
 */
var cmd_memo = {};

/*
 * Rolling log of git shell commands and their responses.
 */
var git_log = [];
...
function append_log(entry) {
    git_log.push(entry);
    if (git_log.length > MAX_LOG_ENTRIES) {
        git_log.shift();
    }
}

function get_git_log() {
    return git_log.slice();
}

function invalidate_cmd_memo(repo) {
    cmd_memo[repo] = {};
}
 ...
 * saved as last_promise for further chaining.  If the process throws, then a new
 * chain is started.
 */
var queue_cmd = (function () {
    var last_promise = Promise.resolve()
    return function (repo, cmd, catch_error) {
        if (last_promise.PromiseStatus) {}
        last_promise = last_promise.then(function () {
            return new Promise(function (resolve, reject) {
                console.log('/* ' + JSON.stringify(repo) + "  :  " + cmd + ' */');
                if (cmd_memo[repo] && cmd_memo[repo][cmd]) {
                    append_log({ timestamp: new Date().toISOString(), repo: repo, cmd: cmd, result: cmd_memo[repo][cmd] });
                    resolve(cmd_memo[repo][cmd]);
                } else {
                    child_process.exec(cmd, {cwd: repo}, function (err, stdout, stderr) {
                        if (err) {
                            console.log(err);
                            append_log({ timestamp: new Date().toISOString(), repo: repo, cmd: cmd, error: err.message || String(err) });
                            reject(err);
                        } else {
                            var result = stdout.replace(/\n+$/, '');
                            if (!cmd_memo[repo]) cmd_memo[repo] = {};
                            cmd_memo[repo][cmd] = result;
                            append_log({ timestamp: new Date().toISOString(), repo: repo, cmd: cmd, result: result });
                            resolve(result);
                        }
                    });
                }
...
function graph(repo, settings) {
    settings = settings || {};
    settings.draw_type = settings.draw_type || 'dot';
    settings.branches = settings.branches || ['main'];
    settings.commits = (settings.commits || []).filter(function (commit) {
        return /^[0-9a-f]{40}$/i.test(commit);
    });
    if (settings.branches.indexOf('HEAD') === -1) {
        settings.branches.push('HEAD');
    }
    settings.bgcolor = settings.bgcolor || {'main': '#eeeeee', 'origin/main': '#eeeeee'};
    ...
    return Promise.all([queue_cmd(repo, 'git tag -l'), queue_cmd(repo, 'git branch -a')])
    .then(function (branches) {
        branches = branches.join('\n');
        settings.branches = settings.branches.filter(function (branch) {
            return branches.indexOf(branch) > -1;
        });
        var rev_targets = settings.branches.concat(settings.commits);
        return queue_cmd(repo, 'git merge-base --octopus ' + rev_targets.join(' '));
    })
    ...
    .then(function (base_commit) {
        ...
        if (settings.include_forward) {
            return queue_cmd(repo, 'git for-each-ref --contains ' + base_commit + ' --format="%(refname:short)"')
            .then(function (branches) {
    ...
    .then(function (branches) {
        ...
        var rev_targets = settings.branches.slice();
        settings.commits.forEach(function (commit) {
            if (rev_targets.indexOf(commit) === -1) {
                rev_targets.push(commit);
            }
        });
        var rev_list_cmds = rev_targets.map(function (branch) {
            // The trailing " --" forces git to parse branch names as revisions,
            // avoiding collisions with working-tree paths of the same name.
            return queue_cmd(repo, 'git rev-list --max-parents=0 ' + branch + ' --')
            .then(function (first_rev) {
                var first_carot = first_rev == vars.base_commit
                    ? ''
                    : carot;
                return queue_cmd(repo, 'git rev-list --parents --first-parent --pretty=oneline ' + branch + ' ' + first_carot + vars.base_commit + first_carot + ' --')
    ...
    .then(function (rev_lists) {
        ...
        return queue_cmd(repo, 'git for-each-ref refs --format="%(refname) %(refname:short) %(objectname) %(*objectname)"')
        .then(function (refs) {
            return refs.split('\n')
    ...
    .then(function (refs) {
        vars.refs = refs;
        return Promise.all([queue_cmd(repo, 'git symbolic-ref --quiet HEAD', true), queue_cmd(repo, 'git rev-parse HEAD')]);
    })
    .then(function (HEAD) {
        vars.HEAD = HEAD;
        ...
        dot += '  graph [layout=' + settings.draw_type + ' splines=splines rankdir=' + settings.rankdir + ' bgcolor="#ffffff" title="GitLost"]\n';
...
module.exports = {
    graph: graph,
    queue_cmd: queue_cmd,
    invalidate_cmd_memo: invalidate_cmd_memo,
